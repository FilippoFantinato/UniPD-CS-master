...
void setupLP(CEnv env, Prob lp)
{
	...
	for (int i = 0; i < I; i++)
	{
		for (int j = 0; j < J; j++)
		{
			if ( C[i*J+j] > od_cost_max ) continue;			///EXT1
			
			char xtype = 'I';
			double lb = 0.0;
			double ub = CPX_INFBOUND; // --///EXT1 with variables fixed to 0.0-- C[i*J+j] <= od_cost_max ? CPX_INFBOUND : 0.0;
	...
	for (int j = 0; j < J; j++)
	{
		std::vector<int> idx;//(I);							///EXT1
		std::vector<double> coef;//(I, 1.0);				///EXT1
		char sense = 'G';
		for (int i = 0; i < I; i++)
		{
	...
	for (int i = 0; i < I; i++)
	{
		std::vector<int> idx;//(J);							///EXT1
		std::vector<double> coef;//(J, 1.0);				///EXT1
		char sense = 'L';
		for (int j = 0; j < J; j++)
		{
			if ( map_x[i][j] < 0 ) continue;				///EXT1
	...
	for ( int i = 0 ; i < I ; ++i ) {
		for ( int j = 0 ; j < J ; ++j ) {
			if ( map_x[i][j] < 0 || map_y[i][j] < 0 ) continue;	///EXT1
	...
	// print (debug)
	CHECKED_CPX_CALL( CPXwriteprob, env, lp, "ironrods_map_extensions.lp", NULL );
...
int main (int argc, char const *argv[])
{
	try
	{
		...
		///EXT1 //if (n != 2*I*J+1) { throw std::runtime_error(std::string(__FILE__) + ":" + STRINGIZE(__LINE__) + ": " + "different number of variables"); }
